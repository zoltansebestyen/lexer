...
class Lexer {
    ...
    func lookahead(offset: Int) -> Character? {
        if offset < distance(cursor, src.endIndex) {
            return src[advance(cursor, offset)]
        }
        return nil
    }
    ...
    func lookaheadUpTo(length: Int) -> String {
        let actualLength = min(length, distance(cursor, src.endIndex))
        let end = advance(cursor, actualLength)
    ...
    func lexPunct() -> Token? {
        ...
        for (idx, op) in enumerate(ops) {
            let candidate = lookaheadUpTo(count(op))
            if candidate == op {
                let value = stepByAtMost(count(op))
    ...
    func nextToken() -> Token? {
        ...
        for (index, fns) in enumerate(lexers) {
            if fns.predicate() {
                return fns.extract()
            }
        }
...
let source = NSString(contentsOfFile: fname, encoding: NSUTF8StringEncoding, error: nil)!
